...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('order_items', function (Blueprint $table) {
            ...
            $table->string('inventory_id'); // bricklink inventoryId
            $table->string('order_id'); // Foreign key to orders table
            $table->string('item_no'); // bricklink item number
            ...
            $table->string('color_id')->nullable(); // bricklink color id
            $table->string('color_name')->nullable(); // Color of the item in the order
            $table->string('condition'); // Condition of the item in the order
            $table->string('category_id'); // Condition of the item in the order
            $table->string('category_name'); // Condition of the item in the order
            $table->string('type'); // type of the item
            $table->timestamps(); // Adds `created_at` and `updated_at` columns
            // Add more column definitions as needed
        });
    }
